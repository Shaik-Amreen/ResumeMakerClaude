...
router.post('/generate-resumes', async (req: Request, res: Response) => {
  try {
    ...
    const pending = await Job.countDocuments({ status: { $in: ['scraped', 'failed'] } });
    ...
    res.json({
      message: `Resume generation started for up to ${count} scraped/failed job(s) — one by one (${resumeAgentLabel()}).`,
      count,
    });
  } catch (error) {
