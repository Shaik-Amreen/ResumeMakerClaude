...
export function buildResumeSystemPrompt(ctx: ResumeJobContext): string {
  ...
  return [
    ...
    'Fill the line so the 1-page resume looks dense/complete (not short stubs). Drop filler; rewrite shorter if needed.',
    'VOICE: human template style (built/wrote/shipped/used…). NO AI traces (leveraged/utilized/spearheaded/cutting-edge/seamless/…).',
    'ADAPTABLE & EVIDENCE (RULE 1): EVERY required JD skill (including Kubernetes, Docker, Helm, Kafka, Microservices, Terraform, GraphQL, etc.) MUST be written into an Experience bullet (Infobell/ASI) or Key Project bullet with concrete evidence. Listing a required skill ONLY under Skills does NOT count (0% match score).',
    'INFOBELL & PROJECTS VERIFIED SKILLS: Kubernetes, K8s, Docker, Helm, Microservices, Kafka, Terraform, Jenkins, Prometheus, Grafana, Spring Boot, Node.js, React, React Native, Redis, PostgreSQL/MySQL, AWS (ECS/EKS/Lambda/S3), CI/CD, REST APIs, GraphQL, C# are explicitly verified under Infobell IT Solutions & Key Projects.',
    'SAME-CONCEPT SWAP OK: Map JD terminology (e.g. Kubernetes/EKS/Helm, Kafka/RabbitMQ, Postgres/MySQL, Express/Fastify) into Infobell or Project bullets with evidence.',
    ...
    '',
    '=== CAREER KNOWLEDGE BASE (verified facts; never invent beyond this) ===',
    careerKb,
    '',
    ...
    '□ R17: workflow complete; R20 short title; R21 3–5 ONE complete line/job (80–95 plain chars, ends with ".")',
...
export function buildResumeUserPrompt(jobDescription: string): string {
  ...
  return [
    ...
    '3) Experience (R21): 3–5 ONE complete line each (target 80–95 plain chars). Full sentence + period; never mid-phrase cut. Amazon first; no Redbee.',
