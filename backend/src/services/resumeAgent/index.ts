...
export async function generateResumeLatex(ctx: ResumeJobContext): Promise<ResumeGenerationResult> {
  ...
  if (provider === 'claude-code') {
    try {
      ...
      // Do not silently fall back to OpenRouter — user asked for Claude.
      // Only fall back to local Ollama if Claude auth/quota fails and Ollama is available.
      if (shouldFallbackFromClaudeCode(err)) {
        console.warn('Claude Code auth failed — trying local Ollama (no OpenRouter fallback)…');
        try {
          return await generateResumeWithOllamaApi(ctx);
        } catch (ollamaErr) {
          throw new Error(
            `${err instanceof Error ? err.message : String(err)} ` +
              `(Ollama fallback also failed: ${ollamaErr instanceof Error ? ollamaErr.message : ollamaErr})`
          );
        }
      }
      throw err;
