...
export async function runResumePipelineQueue(
  ...
  for (let i = startIndex; i < jobIds.length; i++) {
    ...
    try {
      ...
      console.error(`Resume failed for ${jobIds[i]} — continuing queue:`, msg);
      appendTaskLog(`❌ Resume ${i + 1}/${jobIds.length} failed: ${msg.slice(0, 200)}`);
      if (/OmniRoute unreachable|ECONNREFUSED|fetch failed/i.test(msg)) {
        consecutiveUnreachable += 1;
        if (consecutiveUnreachable >= 2) {
          appendTaskLog(
            '⏹ Stopping queue — OmniRoute is down (127.0.0.1:20128). Start OmniRoute, then Generate resumes again.'
          );
          break;
        }
      } else {
  ...
  if (failed > 0) {
    appendTaskLog(`Finished with ${failed} failed resume(s) out of ${jobIds.length}.`);
  }
}

/** Generate resumes one-by-one for scraped + failed jobs (retry failures). */
export async function runResumesForScrapedJobs(options?: {
  ...
  try {
    if (config.resumeAgent.provider === 'openrouter') {
      const { isResumeLlmReachable } = await import('./resumeAgent/openRouterProvider');
      const ok = await isResumeLlmReachable();
      if (!ok) {
        const base = config.resumeAgent.openRouter.baseUrl;
        abortTask(
          `OmniRoute/OpenRouter unreachable at ${base}. Start OmniRoute (port 20128), then click Generate resumes again.`
        );
        throw new Error(`OmniRoute unreachable at ${base}`);
      }
      appendTaskLog(`LLM reachable at ${config.resumeAgent.openRouter.baseUrl}`);
    }

    const query = Job.find({ status: { $in: ['scraped', 'failed'] } }).sort({ createdAt: 1 });
    const jobs = limit ? await query.limit(limit) : await query;
    const jobIds = jobs.map((j) => j.id);
    const scrapedN = jobs.filter((j) => j.status === 'scraped').length;
    const failedN = jobs.filter((j) => j.status === 'failed').length;
    ...
    setTaskPhase(
      'resumes',
      `Generating ${jobIds.length} resume(s) (${scrapedN} scraped + ${failedN} failed), one by one…`
    );
    ...
    if (shouldAbortScrape()) {
      ...
      finishTask(
        `Resume queue complete — ${jobIds.length} job(s) processed (${scrapedN} scraped + ${failedN} failed).`
      );
    }
    ...
    setTaskError(err);
    const msg = err instanceof Error ? err.message : String(err);
    if (!/OmniRoute unreachable/i.test(msg)) {
